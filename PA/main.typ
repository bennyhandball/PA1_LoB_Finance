...
Um die Forschungsfrage, hinsichtlich der Anforderungen, beantworten zu können und eine Handlungsempfehlung abgeben zu können, soll nach der Erhebung und Analyse der Anforderungen die Standarderweiterung implementiert werden und die Implementierung dokumentiert und dargestellt werden. 
...
Hieran orientiert sich auch diese wissenschaftliche Arbeit. Dabei wurden Interviewleitfäden zur Orientierung definiert, von denen während des Gesprächs abgewichen werden kann. Die Interviewleitfäden befinden sich im Anhang der Arbeit. 
//Abschnitt kontrolliert
...
Es wurden Experten der Beratung für einen betriebswirtschaftlichen prozesualen Einblick und Kunden für eine Kundenperspektive befragt. Für diesen Zweck wurden zwei unterschiedliche Interviewleitfäden erstellt, die eine Schnittmenge haben, jedoch einen anderen Fokus legen. Alle Fragen dienen dabei zur Unterstützung der Beantwortung der Forschungsfrage. 
//Abschnitt kontrolliert
...
Da es sich bei der vorliegenden Erweiterung um eine Erweiterung auf dem Business Accelerator Hub handelt, ist diese Erweiterung nicht für die breite Masse an Kunden bestimmt. Aus diesem Grund stehen für diese Standarderweiterung keine Preise zur Verfügung, weshalb die Anforderung hinsichtlich der Kostentransparenz nicht analysiert werden kann. Dies gilt ebenfalls für die Anforderung der Standardisierung der Erweiterung. Die Anforderung "Einfachheit der Benutzung" ist subjektiv und bedarf einer individuellen Beurteilung. Zudem kann diese Anforderung nicht anhand der Implementierung beurteilt werden. 
...
<budget_approval>
In diesem Kapitel wird der Prozess der Projektbudgetgenehmigung genauer erläutert werden. Die Abbildung zeigt die Zielarchitektur für den Genehmigungsprozess (@budget_approval). Der zentrale Einstiegspunkt für Apps, auf mobilen und Desktop-Geräten in einem SAP-System, stellt das Fiori Launchpad dar @SAP_FLP_Einstiegspunkt (@Einstiegspunkt_FLP_ED). 
...
Die Voraussetzung, dass ein Projekt überhaupt budgetiert werden kann, ist die eigentliche Erstellung eines Projekts mit #acr("PSP")-Elementen, wie es im Kapitel 2.2.2 (Grundlagen Projektsystem (PS)) beschrieben wurde. Nachdem der Endbenutzer die App "Import Financial Data, Submit for Approval" geöffnet hat, kann dieser eine Templatedatei herunterladen und diese anschließend ausgefüllt hochladen, um den eigentlichen Workflow des Genehmigungsprozesses zu starten (@Excel_Datei). 
#figure(caption:
...
Diese Informationen müssen in die CSV-Datei eingepflegt werden. Anschließend kann die CSV-Datei in der App "Import Financial Data, Submit for Approval" hochgeladen werden. Dabei wird die CSV-Datei auf Vollständigkeit und Korrektheit geprüft.
//Abschnitt kontrolliert 
...
"assets/Picture_Inbox.jpg"
 ...
 Die erste Möglichkeit stellt die Genehmigung des Projektbudgets dar. Wird das Projektbudget genehmigt, wird dieser Schritt als vollständig abgeschlossen betrachtet, und der nächste Genehmigungsschritt (falls erforderlich) wird eingeleitet. 
