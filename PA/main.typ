...
Mithilfe von Workflows können Geschäftsprozesse automatisiert, standardisiert und effizienter gestaltet werden @SAP_Workflow_Overview. Dabei können Aufgaben, Benachrichtigungen oder Berechtigungen automatisch in einer zuvor definierten Reihenfolge ausgeführt werden @SAP_Workflow_Overview. 
...
Gegenstand dieser Arbeit ist die Standarderweiterung zur Projektbudgetierung in der SAP S/4HANA Public Cloud Version 2408. Nachfolgende Versionen können nicht berücksichtigt werden. Die beschriebenen Funktionaliäten sind bis auf kleinere Einschränkungen auf die SAP S/4HANA Private Cloud übertragbar. Laut dem User Guide (siehe Anhang) wird für die Implementierung der Standarderweiterung das Package mit den ready-to-use Komponenten genutzt, weshalb sich diese Arbeit auf dieses Package bezieht. Andere Packages können dabei nicht berücksichtigt werden.
//Abschnitt kontrolliert.
== Ziel und Gang
...
\
Im Rahmen des theoretischen Teils dieser Arbeit wurde umfangreiche Literatur herangezogen, die primär über Online-Bibliotheken beispielsweise des Springer-Verlags verfügbar sind. Diese Quellen ermöglichten eine fundierte Darstellung der theoretischen Grundlagen. Zusätzlich wurden Fachartikel aus Online-Magazinen herangezogen. Die Informationen dieser Artikel decken sich weitestgehend mit anderen geprüften Online-Quellen, sodass von ihrer Zuverlässigkeit ausgegangen werden kann.

Kritisch betrachtet wurden die von SAP bereitgestellten Materialien, da diese zum Teil auch Marketingaspekte enthalten könnten. Dennoch gelten die meisten Informationen, insbesondere aus dem SAP Help Portal und der offiziellen Dokumentation, als vertrauenswürdig. 
